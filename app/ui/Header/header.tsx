        ...
        <div className='flex flex-row items-center justify-between text-white bg-[#151718] px-[22px] py-[15px] lg:px-[120px] lg:py-[30px]'>
            ...
            <div className='hidden lg:flex flex-row justify-between items-center h-[50px] p-[5px] rounded-[50px] bg-[#ffffff1a] text-[14px] gap-[20px]'>
                {navData.map((item, index) => (
                    <button key={index} className='flex flex-row items-center gap-2 px-[30px] py-[8px] rounded-[50px] hover:opacity-100 hover:bg-white hover:text-black hover:scale-105 transition duration-300 ease-in-out'>
                        <p>{item.title}</p>
            ...
            {
                signed ? <button onClick={handleLogout} className='hidden lg:block px-[41px] py-[12px] gap-[10px] rounded-[16px] border-[2px] text-[14px] font-semibold'>
                    Log Out
                </button> :
